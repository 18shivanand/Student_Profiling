...
def create_projects(request):
		...
		projects.pname = request.POST['pname']
		projects.pid = request.POST['pid']
		projects.startDate = request.POST['start']
		projects.endDate = request.POST['end']
		projects.description = request.POST['description']
		projects.save()
		return HttpResponseRedirect(reverse('resume:projects'))
...
def List(request):
		...
		return render(request, 'resume/list.html', {'users':users,'res':res})

def download(request,id):
	profile = get_object_or_404(UserProfile, user= User.objects.get(pk=id))
	experiences = Experience.objects.filter(user= User.objects.get(pk=id))
	educations = Education.objects.filter(user= User.objects.get(pk=id))
	skills = Skill.objects.filter(user= User.objects.get(pk=id))
	projects = Project.objects.filter(user= User.objects.get(pk=id))
	context = {
